...
### 3.1 (5 points)
```{r}
#My summary table from Milestone 2, exercise 1.2
df <- as_tibble(apt_buildings)

df.summary <- df %>%
  select(no_barrier_free_accessible_units, year_built) %>% # select variables to summarise
  summary() %>% as.tibble(.name_repair= "unique")
df.summary <- as.data.frame(describe(df.summary))

write_csv(df.summary, here::here("output", "df_summary_exported_file.csv"))
